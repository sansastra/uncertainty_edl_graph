...
def load_all_data(timesteps, dim, features, CLASSES):
    ...
    np.random.seed(10)
    path = Path('/home/sing_sd/Desktop/anomaly_detection/PythonCode/Resources/')
    EXTRA_FEATURES = 0
    if CLASSES == 3:
        EXTRA_FEATURES = 1
    column_len = dim * timesteps + EXTRA_FEATURES
    ano_condition = 0.9 * timesteps*dim
    filename = path / 'all_tracks_processed.csv'
    with open(filename, 'rb') as f:
        data = pd.read_csv(f) # pickle.load(f)
    ...
    filename = path / 'data_len_track.pkl'
    with open(filename, 'rb') as f:
...
def load_test_data(timesteps, dim, features, CLASSES):
    np.random.seed(10)
    path = Path('/home/sing_sd/Desktop/anomaly_detection/PythonCode/Resources/')
    EXTRA_FEATURES = 0
    if CLASSES == 3:
    ...
    with open(filename, 'rb') as f:
        data_all_tracks = pickle.load(f)
    vessel_nr = 18  # 28
    overall_data = np.full(shape=(
    int(sum(data_all_tracks[:nr_of_vessels]) + timesteps * len(data_all_tracks) ), column_len),
                           fill_value=-1.0)
    data1 = data.mmsi.unique()
    startIndex = 0
    ...
    for mmsi in data1[nr_of_vessels:]:  # [vessel_nr:vessel_nr+1]: #
        ...
        if decoded_mmsi.shape[0] > timesteps:
            ...
            for slot_index in range(0, decoded_mmsi.shape[0]):  # //
                current_time = datetime.strptime(
                    decoded_mmsi.iloc[slot_index]['date'] + ' ' + decoded_mmsi.iloc[slot_index]['time'],
                    '%m/%d/%Y %H:%M:%S')
                index1 = int((current_time - start_time).total_seconds()) // SAMPLING_TIME
                overall_data[startIndex + index1, 0:dim] = decoded_mmsi.iloc[slot_index, 2:6]
            ...
            for clm_nr in range(1, timesteps):
                ...
                clm_nr * dim:(clm_nr + 1) * dim] = overall_data[
                                                   startIndex + 1: startIndex + data_per_track - clm_nr + 1,
                                                   (clm_nr - 1) * dim:clm_nr * dim]
            # clear last few rows that does not have sufficient data to make ...
            # overall_data[data_per_track[vessel_nr]:data_per_track[vessel_nr] + timesteps,:] = MISS_NUM
            # overall_data[startIndex : startIndex + data_per_track, column_len - 1] = int(mmsi)
            startIndex += data_per_track
    ...
    if (CLASSES == 2):
        ...
        if (len(index_zero) > len(ind_nonzero)):
            ...
            new_data[0:len(ind_nonzero) // 4, 0] = np.random.uniform(11, 11.4, len(ind_nonzero) // 4)
            new_data[0:len(ind_nonzero) // 4, 1] = np.random.uniform(54, 55, len(ind_nonzero) // 4)
            new_data[len(ind_nonzero) // 4:len(ind_nonzero) // 2, 0] = np.random.uniform(12.4, 13,
                                                                                         len(ind_nonzero) // 2 - len(
                                                                                             ind_nonzero) // 4)
            new_data[len(ind_nonzero) // 4:len(ind_nonzero) // 2, 1] = np.random.uniform(54.4, 55,
                                                                                         len(ind_nonzero) // 2 - len(
                                                                                             ind_nonzero) // 4)
            overall_data[index_zero[0:len(ind_nonzero) // 2], 0:dim] = new_data
        else:
...
def load_test_data_new(timesteps, dim, features, CLASSES):
    np.random.seed(10)
    nr_of_vessels = -1
    SAMPLING_TIME = 3
    WINDOW = [11.6, 54.3, 12.6, 54.5]
    SOG_LIMIT = [3, 30]
    NAV_STATUS = 0.0
    SHIP_TYPES = [0, 90]

    # headers = ['x', 'y', 'cog', 'sog', 'time', 'mmsi', "nav_status", "ship_type", "destination"]
    path = Path('/home/sing_sd/Desktop/evidential/src/resources/')
    filename = path / 'ais_data_rostock_2019_processed.csv'
    with open(filename, 'rb') as f:
        data = pd.read_csv(f)
    ...
    # if np.isnan(data["ship_type"]).any():
    #     print("error")
    ...
    overall_data = np.full(shape=(total_rows_data, column_len), fill_value=-1.0)
    ...
    for mmsi in data1[:nr_of_vessels]:  # [vessel_nr:vessel_nr+1]: #
        ...
        if decoded_mmsi.shape[0] > timesteps:
            ...
            decoded_mmsi['time_idx'] = decoded_mmsi.iloc[0]['time']
            decoded_mmsi['time_idx'] = (decoded_mmsi['time'] - decoded_mmsi['time_idx']) // SAMPLING_TIME
            overall_data[np.array(startIndex + decoded_mmsi['time_idx'], dtype=np.int), 0:dim] = np.array(decoded_mmsi[features])

            # shift from top and put on remaining columns
            for clm_nr in range(1, timesteps):
                overall_data[startIndex: startIndex + data_per_track - clm_nr, clm_nr * dim:(clm_nr + 1) * dim] = \
                    overall_data[startIndex + 1: startIndex + data_per_track - clm_nr + 1, (clm_nr - 1) * dim:clm_nr * dim]
