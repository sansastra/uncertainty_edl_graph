...
def load_test_data_new(timesteps, dim, features, CLASSES): # this is with dummy number, or time
    nr_of_vessels = -1
    SAMPLING_TIME = 3
    ...
    np.random.seed(10)
    try:
        with open("../resources/ais_data_rostock_2019_processed.csv", 'rb') as f:
            data = pd.read_csv(f)
    ...
    startIndex = 0
    for mmsi in data1[:nr_of_vessels]:
        decoded_mmsi = data[data['mmsi'] == mmsi]
        decoded_mmsi = decoded_mmsi.reset_index(drop=True)
        if decoded_mmsi.shape[0] > timesteps:
