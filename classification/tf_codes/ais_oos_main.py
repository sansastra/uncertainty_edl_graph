...
from load_data_last_seen import load_all_data, load_test_data
...
for val in range(nr_iter):
    ...
    plt.legend()
    plt.pause(0.001)
    plt.savefig(results_dir + "oos_train_val_loss_epochs_" + str(epochs) + ".png")
    ...
    evidence = np.array(tf.clip_by_value(evidence, clip_value_min=-10000, clip_value_max=100))
    if evidence.any() == 0 or np.isnan(evidence.any()):
        print("evidence has 0 or nan")
    if evidence.min() < 0:
    ...
    plt.savefig(results_dir + "oos_uncertainty_val_epochs_" + str(epochs) + ".png")
