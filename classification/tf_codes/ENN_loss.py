...
def build_evidential_mse(lambda_callback: ENN_lambda_update):
    def evidential_evidential_mse(y_true, logits):
        ...
        logits = tf.cast(logits, tf.float64)
        # alpha_c = tf.exp(logits) + 1
        # tf.print("alpha_c_exp = ", alpha_c)
        alpha_c = tf.nn.relu(logits) + 1
        # alpha_c = tf.clip_by_value(alpha_c, clip_value_min=-10000, clip_value_max=300) # clip_value_min=10e-25, clip_value_max=10e25
        # tf.print("alpha_c_relu = ", alpha_c)
        S = tf.reduce_sum(alpha_c, axis=-1, keepdims=True)
        # tf.print("evidence= ", S)
        E = alpha_c - 1
        m = alpha_c / S
